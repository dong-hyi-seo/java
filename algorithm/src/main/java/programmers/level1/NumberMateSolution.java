 ...
 * 지문에 설명된 예시와 같습니다.
 */
public class NumberMateSolution {
    ...
    public String solution(String X, String Y) {
        ...
        for (String tmpX : X.split("")) {
            System.out.println("Integer.parseInt(tmpX) = " + Integer.parseInt(tmpX));
            cntX[Integer.parseInt(tmpX)]++;
        }
        System.out.println("cntX = " + Arrays.toString(cntX));
        System.out.println("===================");
        for (String tmpY : Y.split("")) {
            System.out.println("Integer.parseInt(tmpY) =  " + Integer.parseInt(tmpY));
            cntY[Integer.parseInt(tmpY)]++;
        }
        System.out.println("cntY = " + Arrays.toString(cntY));
     ...
     * 시간초과발생
     */
    public String mySolution(String X, String Y) {
        ...
        for (int i=0; i < X.length(); i++) {
            int xNum = Character.getNumericValue(X.charAt(i));
            for (int a=0; a < yArr.length; a++) {
                int yNum = Integer.parseInt(yArr[a]);
                if (xNum == yNum) {
                    list.add(xNum);
                    yArr[a] = "-1";
                    break;
