...
public class StringCompression {
    ...
    public int mySolution(String s) {
        ...
        int length = s.length();
        System.out.println("question string = " + s);
        System.out.println("question string length = " + length);

        for (int i=1; i <= length/2+1; i++) { //최대 잘라볼수있는 길이
            String str = new String();
            String target = s.substring(0, i); //압축할 문자 잘라낸다
            System.out.println("=====> target : " + target);
            for (int start = i; start < length; start+=i) {
                System.out.println("i = " + i + " // start = " + start + " // test = " + s.substring(i) );
            }
        }
